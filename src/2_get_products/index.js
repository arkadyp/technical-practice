...
/*-----------------------------------------------------------------------------------------*/
function product_at_every_index(arr) {
    // Calculate products of all indicies before given number
    var product_not_including_index = [];
    var product = 1;
    for (let i = 0; i < arr.length; i++) {
        product_not_including_index.push(product);
        product *= arr[i];
    }

    product = 1;
    for (let i = arr.length - 1; i >= 0; i--) {
        product_not_including_index[i] *= product;
        product *= arr[i];
    }

    return product_not_including_index;
}
...
console.log(result); // [84, 12, 28, 21]
